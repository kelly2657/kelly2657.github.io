...
특성 `Country` 와 `Latitude/Longitude`가 상관 관계이기 때문에 정확도는(약 96%) 나쁘지 않게 나온다.
...
모델이 경도와 위도에 따른 국가 코드 __1__(캐나다)을 반환함을 볼 수 있다.
...
📋: 파일, 📁: 폴더
1. 📋`ufo-model.pkl`과 📋`notebook.ipynb` 옆에 📁`web-app`을 만든다.

사진 입력(mlasg9_1.png)

2. 📁`web-app` 안에 📁`static`(내부에 📁`css`)과 📁`templates`을 만든다.

사진 입력(mlasg9_2.png)

사진 입력(mlasg9_2_2.png)


3. 📁`web-app` 안에 📋`requirements.txt`을 만든다.  
...
5. `pip install` 명령으로 `requirements.txt`에 나열된 라이브러리를 설치한다.
```
...
6 - 2. 📁`templates`안에 📋`index.html`를 만든다.
...
6 - 3. 📁`static/css`안에 📋`styles.css`를 만든다.
